...
The theory of spatial point patterns is discussed in great detail in [@baddeleySpatialPointPatterns2015]. The book has an accompanying package called  which offers great functionality to the theoretical concepts described in the book [@baddeleySpatstatPackageAnalyzing2005]. This chapter relies heavily on both publications.
...
An option to define arbitrary observation windows is to base them directly on the intensity of points. We do this using . This package calculates an observation window based on an intensity threshold of points in space.
